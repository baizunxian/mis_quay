...
class Quay_crane(models.Model):
    ...
    voy = models.ManyToManyField("information.Voyage", through='voyage_qc', null=True)


# 岸桥_航次表
class voyage_qc(models.Model):
    voy = models.ForeignKey('information.Voyage', '航次表')
    qc = models.ForeignKey('Quay_crane', '岸桥表')
    start_plan = models.DateTimeField(verbose_name="开始占用时间", null=True)
    end_plan = models.DateTimeField(verbose_name="结束占用时间", null=True)
...
class AGV(models.Model):
    ...
    state_choices = (
        (1, "作业"),
        (2, "空闲"))
    agv_id = models.IntegerField(primary_key=True, verbose_name="AGV编号")
    situation = models.IntegerField(verbose_name="设备情况", choices=situation_choices)
